...
async function volumeAction(args, callback) {
  let volume = args && args.options && args.options.volume
  if (volume) {
    try {
      const v = await Player.setVolume(volume)
      this.log(`System Volume: ${v}`)
    } catch (error) {
      this.log(`Error Setting System Volume: ${error}`)
    } finally {
      callback()
    }
  } else {
    try {
      ...
      this.log(`Error Getting System Volume: ${error}`)
    } finally {
      callback()
    }
  }
...
[
  ...
  {
    command: 'volume',
    description: 'Get or set volume for the system. When you provide a float value between 0.0 and 1.0 the volume will attempt to be set to this value. Calling this command without a value will get and print the current system volume.',
    action: volumeAction,
  },
  {
    command: 'play',
